...
from pipeline.generators.image import asset_digest, sniff_image
...
from pipeline.scenes import SceneBook, SceneRecord
...
def prepare_publication(
    book: SceneBook, world: WorldModel, sources_dir: Path, root: Path
) -> Publication:
    ...
    layer_files, layers = _layers(world)
...
def write_publication(publication: Publication, media_dir: Path) -> Path:
    for copy in publication.scene_files:
...
def _scene_entry(scene: SceneRecord, root: Path) -> tuple[Scene, MediaCopy]:
    ...
    source = root / scene.pin.path
    if not source.is_file():
        raise PublishRefused(f"{scene.id}: pinned image {scene.pin.path} does not exist")
    data = source.read_bytes()
    if asset_digest(data) != scene.pin.asset_digest:
        raise PublishRefused(
            f"{scene.id}: {scene.pin.path} no longer matches pinned digest {scene.pin.asset_digest}"
        )
...
def _layers(world: WorldModel) -> tuple[tuple[LayerFile, ...], tuple[LayerManifest, ...]]:
