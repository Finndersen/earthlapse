...
describe('isOrbClick', () => {
  ...
  it('is a click just under the threshold', () => {
    expect(isOrbClick({ x: 0, y: 0 }, { x: ORB_CLICK_DRAG_THRESHOLD_PX - 0.1, y: 0 })).toBe(true)
  })
  ...
  it('respects a custom threshold', () => {
    expect(isOrbClick({ x: 0, y: 0 }, { x: 10, y: 0 }, 20)).toBe(true)
    expect(isOrbClick({ x: 0, y: 0 }, { x: 10, y: 0 }, 5)).toBe(false)
  })
})
