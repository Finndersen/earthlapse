 ...
 * come from `drawnEmpireLabels` (largest member, ranked by area, capped, thinned so no two
 * overlap on screen), and fade with the same presented crossfade the territory texture
 ...
 * Labels rest at `EMPIRE_LABEL_REST_OPACITY`; the hovered or selected lineage's draws at full.
 */
...
import { EMPIRE_LABEL_REST_OPACITY, empireColour } from './empireStyle'
...
/** The labels drawn for one frame: `empireLabelsAt`'s ranked, capped set with every label whose
 *  box overlaps a larger lineage's dropped. `worldPerPx` is one CSS pixel in object space. The hit
 *  test reads the same set, so a hidden label's anchor never answers for a drawn neighbour. */
export function drawnEmpireLabels(frame: EmpireFrame, cap: number, unfold: number, worldPerPx: number): EmpireLabel[] {
...
export function EmpireLabels({ presented, cap, unfold, radius, highlight }: EmpireLabelsProps) {
  ...
  const placed = useMemo(() => {
    ...
    const add = (frame: EmpireFrame, weight: number): void => {
      if (weight <= 0) return
      for (const label of drawnEmpireLabels(frame, cap, unfold, worldPerPx)) {
    ...
    <group ref={groupRef}>
      {placed.map(({ key, label, opacity }) => (
        <GlobeLabel
          ...
          lat={label.lat}
          lon={label.lon}
          opacity={opacity * (highlight.has(label.lineage) ? 1 : EMPIRE_LABEL_REST_OPACITY)}
          unfold={unfold}
          radius={radius}
          groupRef={groupRef}
