...
const LOG_SPACED_T: readonly number[] = [0, 1, 1e2, 1e4, 1e6, 1e8, 1e9, EARTH_FORMATION]

/** Absolute floor plus a relative term: t spans ten orders of magnitude. */
...
describe.each([
  ...
  it('round-trips t → u → t, maps u 0/1 to the oldest/newest edge, and runs oldest to newest', () => {
    const scale = makeScale()
    for (const t of LOG_SPACED_T) expectRoundTrips(t, scale.fromUnit(scale.toUnit(t)))
...
describe('createSymlogScale', () => {
  ...
  it('gives recorded history (last 10 ka) a legible, non-sub-pixel share of the axis at full zoom-out', () => {
    ...
    // 0.5% of a 4000px axis is 20px.
    expect(fractionForRecordedHistory).toBeGreaterThan(0.005)
  })

  it('accepts an explicit knee override instead of deriving one from the window', () => {
...
describe('symlogKnee', () => {
  it('is exactly SYMLOG_C at and above the adaptive threshold', () => {
  ...
  it('stays positive for a zero-span window', () => {
    expect(symlogKnee([0, 0])).toBeGreaterThan(0)
  })

  it('gives a small recent section a legible share of its own axis, unlike a fixed knee', () => {
    const holocene: TimeWindow = [0, 11_725]
...
describe('blendScales', () => {
  it('interpolates toUnit linearly in k, clamped to the endpoints', () => {
    const symlog = createSymlogScale(FULL_DOMAIN)
    const linear = createLinearScale(FULL_DOMAIN)
    const t = 5e7
    expect(blendScales(symlog, linear, 0.5).toUnit(t)).toBeCloseTo((symlog.toUnit(t) + linear.toUnit(t)) / 2, 9)
