...
import { DECODE_AHEAD_SCENES, FETCH_AHEAD_SCENES, planScenePrefetch, type PlaybackHeading } from './prefetch'
...
describe('planScenePrefetch', () => {
  it('reads ahead in the direction of playback, capped, and falls back to one either side while paused', () => {
    expect(planAt(300, null)).toEqual({ decode: [29, 31], fetch: [] })
    expect(planAt(300, { t: 300, horizonT: 300 })).toEqual({ decode: [29, 31], fetch: [] })

    expect(planAt(300, { t: 300, horizonT: 250 })).toEqual({ decode: [29, 28, 27, 31], fetch: [26, 25, 32] })
    expect(planAt(300, { t: 300, horizonT: 350 })).toEqual({ decode: [31, 32, 33, 29], fetch: [34, 35, 28] })
