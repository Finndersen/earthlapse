 ...
 */
export function usePresentedSceneMix(target: SceneMix): SceneMix {
  ...
  useEffect(() => {
    ...
    if (rafRef.current !== null || converged(presentedRef.current, target)) return
    ...
    const tick = (now: number): void => {
      ...
      const next = step(presentedRef.current, targetRef.current, dtSeconds)
      presentedRef.current = next
      setPresented(next)

      if (converged(next, targetRef.current)) {
        rafRef.current = null
        lastFrameRef.current = null
        return
      }
