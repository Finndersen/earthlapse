...
from pipeline.generators import gate
...
from pipeline.generators.image import ImageInfo, sniff_image
...
def _node(node_id: str = "scene", reference: Path | None = None) -> AssetNode:
    inputs: dict[str, str] = {"prompt": "a shore"}
...
def test_conditioned_request_names_pinned_model_and_carries_anchor_inline(tmp_path: Path) -> None:
    ...
    _generator(handler, tmp_path / "spend.json").render(_node(reference=tmp_path / "anchor.png"))
